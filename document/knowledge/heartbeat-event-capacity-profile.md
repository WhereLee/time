...
**成本侧观察（日志放大）**：6056 个过闸 401 各产生一条 warn（RRExceptionHandler 状态透传）——60 秒内 warn.log 灌入 6056 行/约 1.3MB，即 **≈100 行/秒、1.3MB/分钟**。限流保护了业务层与 MQ，但**日志侧无配额**：把攻击速率控制在闸内即可持续放大日志（磁盘/IO 压力）。生产形态对策=日志采样或聚合计数（演示环境未做，如实声明）。**已修并实测：批次8 P3 `LogThrottle`（per-key 5s 窗口+抑制计数，接入状态透传与限流拒绝路径）——S3 同场景重跑 6056 → 32 行（-99.5%）、响应分布零变化（401=6035/429=925）；见 document/fixes/reject-log-flood-throttling.md 与 scripts/verify/batch8/。**
